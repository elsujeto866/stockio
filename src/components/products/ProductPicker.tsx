 ...
 *     gated by {open && <PickerContent />} — PickerContent mounts fresh each
 *     time the dialog opens, which resets query state without a secondary
 *     useEffect (avoids react-compiler "setState in effect body" lint error).
...
// ---------------------------------------------------------------------------
// PickerContent — owns query state, remounts on each open cycle
// ---------------------------------------------------------------------------
// Separating this into its own component means when open→false the sub-tree
// unmounts and query resets automatically — no secondary useEffect needed.
...
function PickerContent({ products, photoUrls, onClose, onSelect }: PickerContentProps) {
...
// ---------------------------------------------------------------------------
// ProductPicker — stable dialog shell + feature-detected open/close
// ---------------------------------------------------------------------------
export function ProductPicker({
  ...
  return (
    ...
    >
      {/* PickerContent is conditionally mounted — remounts on each open cycle.
          This resets query state without a secondary useEffect. */}
      {open && (
        <PickerContent
          products={products}
          photoUrls={photoUrls}
